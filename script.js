/* Logic and Data Fetching for SteoType (INFINITE SCROLL + REVERSED LOGIC FIXED) */

const SPREADSHEET_URL = "/api/stok";
let allData = []; // Master data dari server
let currentFilteredData = []; // Data setelah difilter & disortir
let displayLimit = 12; // Jumlah awal yang ditampilkan
...
function openCartModal() {
  ...
  const formattedTotal = formatHarga(totalHarga);
  ...
  cartFooter.innerHTML = `
    <div class="flex justify-between items-center mb-4">
      ...
      <span id="cartTotal" class="text-xl font-bold text-blue-400">${formattedTotal}</span>
    </div>
...
async function loadData() {
  ...
  try {
    ...
    // Enrich data dengan search string untuk performa pencarian kilat
    ...
    updateCounts();
    applyFilters(); // Filter awal (Ready)
  } catch (e) {
    loader.style.display = 'none';
...
function applyFilters() {
  ...
  // LOGIKA BARU: Tab 'sold' tidak lagi di reverse. 
  // Baris paling atas di Spreadsheet akan jadi item pertama di Website.

  currentFilteredData = filtered;
  displayLimit = 12; // Reset limit setiap kali filter berubah
  renderProducts();
}

// Fungsi Pencarian Terintegrasi (Cepat)
