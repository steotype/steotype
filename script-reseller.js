...
// AMBIL DATA DISKON DARI SESSION
...
// LOGIKA MODAL
function openModal() { document.getElementById('tncModal').classList.remove('hidden'); const b = document.getElementById('modalBackdrop'); const c = document.getElementById('modalContent'); setTimeout(() => { b.classList.replace('opacity-0', 'opacity-100'); c.classList.replace('opacity-0', 'opacity-100'); c.classList.replace('scale-95', 'scale-100'); }, 10); }
function openTncFromCheckout() { closeBuyModal(); setTimeout(() => { openModal(); }, 300); }
...
async function loadData() {
  ...
  try {
    ...
    loader.style.display = 'none';
    grid.innerHTML = '<p class="text-center text-red-400 py-10">Gagal memuat database reseller.</p>';
  }
}
...
function applyFilters() {
  let filtered = [];
  if (currentTab === 'ready' || currentTab === 'sold') { filtered = allData.filter(item => item.status_stok?.toLowerCase() === currentTab); } 
  else if (currentTab === 'mt') { filtered = allData.filter(item => { const htmtVal = item.status_htmt?.toLowerCase() || ''; return item.status_stok?.toLowerCase() === 'ready' && htmtVal.includes('mt') && !htmtVal.includes('ht'); }); } 
  else if (currentTab === 'htmt') { filtered = allData.filter(item => item.status_stok?.toLowerCase() === 'ready' && (item.status_htmt?.toLowerCase() || '').includes('ht')); }
...
function renderProducts() {
  const grid = document.getElementById('productGrid');
  grid.innerHTML = ''; 
  if (currentFilteredData.length === 0) { document.getElementById('noResults').classList.remove('hidden'); return; }
...
function renderBatch() {
  ...
  batch.forEach((item) => {
    ...
    // LOGIKA IKON (DIPERBAIKI)
    const telpText = item.verif_telepon || '-';
    const telpIcon = (telpText.toLowerCase().trim() === 'tidak' || telpText === '-') ? '❌' : '✅';
    const emailText = item.email || '-';
    const emailIcon = (emailText.toLowerCase().trim() === 'tidak' || emailText === '-') ? '❌' : '✅';
    const kesText = item.kesehatan || '-';
    const kesIcon = kesText.toLowerCase().includes('go green') ? '🛡️' : '🚦';
    const kesColor = kesText.toLowerCase().includes('go green') ? 'text-green-400' : 'text-gray-400';
    const htmtLow = (item.status_htmt || '').toLowerCase().trim();
    ...
    const cardHtml = `
      <div class="product-card bg-gray-800 rounded-2xl border ${inCart ? 'border-blue-500 shadow-blue-900/20 shadow-lg' : 'border-gray-700'} fade-in-down overflow-hidden ${!isReady ? 'opacity-60' : ''}" data-username="${item.username}">
        <div class="h-24 bg-gray-900 relative">
           <span class="absolute top-3 left-3 px-2 py-1 bg-black/60 text-gray-300 text-[10px] font-mono rounded border border-gray-700">KODE: ${item.kode}</span>
           <span class="absolute top-3 right-3 px-2.5 py-1 ${isReady ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'} text-[10px] uppercase font-bold rounded">${item.status_stok}</span>
        </div>
        <div class="px-4 pb-4">
          <div class="flex justify-between items-start -mt-10 mb-2 relative z-10">
            <div class="w-[72px] h-[72px] rounded-full border-4 border-gray-800 bg-gray-900 flex items-center justify-center text-gray-600 overflow-hidden"><svg class="w-10 h-10 mt-2" fill="currentColor" viewBox="0 0 24 24"><path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" /></svg></div>
            <div class="mt-12">
              ${isReady ? `<a href="${item.link_akun}" target="_blank" class="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full border border-gray-600 bg-gray-800 text-[13px] font-bold text-white hover:bg-gray-700 transition-colors">Cek Profil</a>` : `<span class="text-gray-500 text-xs font-bold">Privat</span>`}
            </div>
          </div>
          <div class="mb-3">
          ...
          <div class="text-[14px] text-gray-200 mb-3 space-y-1">
             ...
             <p>${htmtIcon} Status: <span class="${htmtColor} font-semibold">${item.status_htmt || '-'}</span></p>
          </div>
          <div class="flex flex-wrap gap-x-4 gap-y-2 text-sm text-gray-400 mb-3">
             <span class="flex items-center gap-1">Bergabung ${item.tahun_akun}</span>
          </div>
          ...
          <div class="mt-2 pt-4 border-t border-gray-700 flex justify-between items-center">
             <div>
                <p class="text-[10px] text-gray-400 uppercase font-bold">Partner Price</p>
                <p class="text-xl font-bold text-blue-500">Rp ${item.hFinalNum.toLocaleString('id-ID')}</p>
                <p class="text-[10px] text-gray-600 line-through">Rp ${item.hAsliNum.toLocaleString('id-ID')}</p>
             </div>
             <div>
              ${isReady ? 
                `<button onclick="toggleCart('${item.kode}', '${item.username}', ${item.hAsliNum}, ${item.hFinalNum})" class="btn-add-cart px-5 py-2.5 ${inCart ? 'bg-blue-600' : 'bg-gray-800 border border-gray-600'} text-white text-sm font-semibold rounded-lg transition">${inCart ? 'Terpilih' : 'Tambah'}</button>` :
...
window.addEventListener('scroll', () => {
  if ((window.innerHeight + window.scrollY) >= (document.body.scrollHeight - 800)) {
    if (!isLoadingMore && displayLimit < currentFilteredData.length) {
      isLoadingMore = true; displayLimit += 12; renderBatch();
    }
  }
});
